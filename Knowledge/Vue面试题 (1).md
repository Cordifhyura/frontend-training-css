...
## 5.6v-for与v-if的优先级那个高?如果同时使用v-for和v-if怎么解决?
...
## 5.8vue组件通信（多说点一共有8个）? 写组件的目的是什么？要解决什么？props选项有啥作用  ?说一下组件化开发的理解?(必问)`*********`?
...
## 5.14 vue如何封装可复用的组件?以及请举例说明你封装过的组件?
...
## 5.16说一下你对keep-alive的理解?以及在项目中如何使用?
...
## 5.17说一下什么是vue过滤器? 有几种?项目中如何使用,请举例说明?
...
## 5.19说一下vue中本地跨域如何解决?线上跨域如何解决?之前前后端分离跨域怎么实现? 第一个请求超时了怎么解决？同时向服务器发送六个请求，但是第一个请求发送成功后，剩余的请求就不需要了，怎么实现？如何保证前端调用接口的安全性？
...
## 5.20 说一下如何对axios进行二次封装?以及api如何封装?
...
## 5.25发送ajax到后端请求数据的过程 , 有哪些必须要设置的参数?
...
## 5.26props设置默认值? props自定义验证函数? default设置对象和普通字符串写法有啥区别  (必问) `***`
...
## 5.27vue项目是打包了一个js文件，一个css文件，还是有多个文件？

```js
vue项目打包是将多个文件合并一个js文件和一个css文件
```

## 5.28vue-cli 工程常用的 npm 命令有哪些?
...
## 5.29vue 中使用了哪些设计模式？订阅者--发布者模式?(90%)  观察者模式、 订阅者模式 `**`
...
## 5.35vue中加载应用太多加载慢，是什么导致的? 如何解决? 路由懒加载? component实现懒加载的原理(必问) 
...
## 5.37element-ui中table中的列表，表头主要有哪些功能?element中table列表表头主要支持那些功能？表格组件有那些方法吗
...
## 5.38图片的上传下载有封装过组件吗? 上传用哪种方式？编码上传用什么方法？
...
## 5.39页面鉴权怎么分配权限? <动态添加路由表>添加和获取动态路由?怎么做权限管理(rbac)? 按钮鉴权是怎么处理的？ 路由权限? 菜单权限? 数据权限?做后台管理系统，权限管理是怎么做的，有哪几个方面
...
## 5.40封装axios的时候一般做什么工作 ? axios是怎么封装的？请求拦截和响应拦截器都是干什么的？ 简单解释下axios? promise与axios的关系 (必问) 
...
## 5.41如果想深度修改 element-ui 样式？怎么修改样式 ？ 如果修改样式不生效，怎么处理？
...
## 5.47cookie和localStorage、sessionStorage的区别 
